...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    console.log("Garden application email sent successfully:", emailResponse);

    // Send confirmation email to applicant
    await resend.emails.send({
      from: "OllyGarden Careers <careers@olly.garden>",
      to: [applicant.email],
      subject: `Thank you for applying to join our Garden - ${jobTitle}`,
      html: `
    ...
    console.error("Error sending garden application email:", error);
    return new Response(
